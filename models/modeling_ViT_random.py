...
class Attention(nn.Module):
    def __init__(self, config):
        ...
        self.top_k = 685
        self.random = 0.625
        self.k = torch.tensor(self.random).cuda()
    ...
    def topK(self,attention_scores,top_k,mask_fb = None):
        # print(attention_scores.shape)
        # random_01 = torch.rand(attention_scores.shape)
        random_01 = torch.cuda.HalfTensor(attention_scores.shape[0],attention_scores.shape[1],attention_scores.shape[2],attention_scores.shape[3]).uniform_()
        # random_01 = random_01.cuda().half()
        
        # k = torch.tensor(self.random).cuda()
        tk = self.k.expand_as(random_01)
        # print(tk.shape)
        
        
        # 小于tk置0
        mask_0 = torch.lt(random_01, tk)
        # 大于等于tk置1
        mask_1 = torch.ge(random_01, tk)
        random_01 = random_01.masked_fill(mask_0, 0).type_as(random_01)
        random_01 = random_01.masked_fill(mask_1, 1).type_as(random_01)
        
        mask_attn = random_01
        # print(mask_attn.shape)
        # mask_attn全局token置1
        mask_attn[:, :, 0] = 1
        mask_attn[:, :, :, 0] = 1
    ...
    def forward(self, hidden_states,mask_fb=None):
        ...
        attention_scores = self.topK(attention_scores,self.top_k,mask_fb)
