...
class MergingLayer(nn.Module):
    ...
    def __init__(self, input_resolution, dim,config, norm_layer=nn.LayerNorm):
        super().__init__()
        self.PatchMerging1 = PatchMerging(input_resolution, dim=dim, norm_layer=nn.LayerNorm)
        self.PatchMerging2 = PatchMerging(input_resolution//2, dim=dim*2, norm_layer=nn.LayerNorm)
        
        self.mergeMLP1 = mergeMlp(config,4)
        self.mergeMLP2 = mergeMlp(config,4)
        self.mergeMLP3 = mergeMlp(config,2)
        self.mergeMLP4 = mergeMlp(config,2)
...
class mergeMlp(nn.Module):
    def __init__(self, config,ratio):
        ...
        # TODO inputsize outputsize
        self.fc1 = Linear(config.hidden_size// ratio, config.transformer["mlp_dim"])
        self.fc2 = Linear(config.transformer["mlp_dim"], config.hidden_size// ratio)
        self.act_fn = ACT2FN["gelu"]
        self.dropout = Dropout(config.transformer["dropout_rate"])
...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        if config.split == 'non-overlap':
            n_patches = (img_size[0] // patch_size[0]) * (img_size[1] // patch_size[1])//16
            ...
            n_patches = ((img_size[0] - patch_size[0]) // config.slide_step + 1) * ((img_size[1] - patch_size[1]) // config.slide_step + 1)//16
            self.patch_embeddings = Conv2d(in_channels=in_channels,
                                        out_channels=config.hidden_size//4,
                                        kernel_size=patch_size,
        ...
        # TODO
        self.MergingLayer = MergingLayer(56, 192,config, norm_layer=nn.LayerNorm)
    def forward(self, x):
        B = x.shape[0]
        ...
        x = self.MergingLayer(x)
        # print("****aft__embedding",x.shape)
...
class Transformer(nn.Module):
    ...
    def forward(self, x,forward_counts,mask=None):
        if forward_counts == 0:
            embeddings_ouputs = self.embeddings(x)
        else:
            embeddings_ouputs = x
        part_encoded, weight_map = self.encoder(embeddings_ouputs,forward_counts,mask)
        return part_encoded, weight_map,embeddings_ouputs
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, smoothing_value=0, zero_head=False):
        ...
        patch_size = _pair(config.patches["size"])
        if config.split == 'non-overlap':
            self.num_tokens = (448 // patch_size[0]) * (448 // patch_size[1])//16
        elif config.split == 'overlap':
            self.num_tokens = ((448 - patch_size[0]) // config.slide_step + 1) * ((448 - patch_size[1]) // config.slide_step + 1)//16
...
# =============================================================================
        forward_counts = 0
        part_tokens, weight_map,embeddings_ouputs = self.transformer(x,forward_counts)
...
# =============================================================================
        ...
        forward_counts = 1
        part_tokens, weight_map,embeddings_ouputs = self.transformer(embeddings_ouputs,forward_counts,mask_no_grad)
...
class CenterLoss(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        return loss
CONFIGS = {
    'HiViT': configs.get_HiViT_config(),
