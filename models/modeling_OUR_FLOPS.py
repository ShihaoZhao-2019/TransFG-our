...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, smoothing_value=0, zero_head=False):
        ...
        if config.split == 'non-overlap':
            ...
            self.num_tokens = ((448 - patch_size[0]) // config.slide_step + 1) * (
                        (448 - patch_size[1]) // config.slide_step + 1)
    ...
    def forward(self, x, labels=None, one_hot_targets=None, local_rank=None):
        ...
        preds_cls = preds_cls.view(-1, self.num_tokens + 1,
                                   self.num_classes)  # [B,1370,200]
