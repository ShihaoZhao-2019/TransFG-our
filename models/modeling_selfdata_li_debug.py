...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, labels=None):
        part_tokens,part_inx = self.transformer(x)
        part_logits = self.part_head(part_tokens[:, 0])

# =============================================================================
#         self.save_image(x,part_inx)
# =============================================================================
    ...
    def save_image(self,image,index):
        ...
        for i in range(0,len(index[0])):
            hang = int(index[0][i]/37)
            lie = (index[0][i]-hang*37)
            ...
            # proposals = proposals.append([x1,y1,x2,y2])
            # print(proposals)    
        
        for i in range(0,len(index[0])):
            cv2.rectangle(image, (proposals[i][0],proposals[i][1]), (proposals[i][2],proposals[i][3]), (255,0,0), 2)
        image_savepath = os.path.join('/data/kb/tanyuanyong/TransFG-master/debug','%s.jpg' % self.num)
        self.num += 1
