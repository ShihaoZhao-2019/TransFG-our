...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, labels=None, one_hot_targets=None,local_rank=None):
        ...
        print('preds_cls',preds_cls.shape)
        print('labels',labels)
        sort_preds_cls = preds_cls.squeeze(0)
        sort_preds_cls = sort_preds_cls[1:,:]
        sort_preds_cls = torch.sort(sort_preds_cls,dim=0,descending=True)[1][:,labels[0]]
        print('sort_preds_cls',sort_preds_cls.shape)
        print('preds_cls',sort_preds_cls[0:100].view(1,-1))
...
# =============================================================================
        ...
        first_pred = torch.argmax(image_level_scores,dim=-1)
        second_pred = torch.argmax(part_logits,dim=-1)
        predict_cls = torch.argmax(preds_cls,dim=-1)
        
        count0 = 0
        count1 = 0
        for i_p in range(0,1370):
            if predict_cls[0,i_p] != labels[0]:
                count0 += 1
                # print(i_p)
                # print('错误',i_p,predict_cls[0,i_p])
         
        print('错误token0',count0)
        print('错误token1',count1)       
        print('first_pred',first_pred)
        print('second_pred',second_pred)
        print('gt_labels',labels)
        
        save_labels = labels
        save_x_cls = x_cls
        save_x_det = x_det
        save_preds_cls = preds_cls
        # save_last_map = last_map
        
        save_labels = np.array(save_labels.detach().cpu())
        save_x_cls = np.array(save_x_cls.squeeze(0).detach().cpu())
        save_x_det = np.array(save_x_det.squeeze(0).detach().cpu())
        save_preds_cls = np.array(save_preds_cls.squeeze(0).detach().cpu())
        # save_last_map = np.array(save_last_map.squeeze(0).detach().cpu())
        
        # self.data_write_csv("./train_loss.csv", Data)
        io.savemat('data.mat',{'labels':save_labels,'x_cls':save_x_cls,'x_det':save_x_det,'preds_cls':save_preds_cls })
        ...
        if labels is not None:
            if self.smoothing_value == 0:
                ...
                loss_wsddn = nn.BCEWithLogitsLoss(reduction='mean') # reduction='sum'
            else:
...
# =============================================================================
# =============================================================================
#             print('cls_token_loss',cls_token_loss)
# =============================================================================
# =============================================================================
#             if local_rank in [-1, 0]:
#                 print('main_loss',part_loss+ contrast_loss)
#                 print('part_loss',part_loss)
# =============================================================================
            # loss = part_loss+WSDDN_loss+cls_token_loss
            loss = part_loss+alpha*(WSDDN_loss+cls_token_loss)+ contrast_loss
            # loss = part_loss+alpha*(WSDDN_loss)+ contrast_loss
