...
class CUB():
    def __init__(self, root, is_train=True, data_len=None, transform=None):
        ...
        if self.is_train:
            ...
            train_img_dict = {}
            for train_img in self.train_imgname:
                if train_img[:3] not in train_img_dict.keys():
                    train_img_dict[train_img[:3]] = []
                train_img_dict[train_img[:3]].append(train_img)
                
            select_rate = 0.5
            self.select_list = {}
            for img_class,img_name in train_img_dict.items():
                # print(img_class,img_name)
                select_num = int(len(img_name)*select_rate)
                select_img = random.sample(img_name,select_num)
                self.select_list[img_class] = select_img
            print('self.select_list',self.select_list)
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            if imgname in self.select_list[imgname[:3]]:
                print(imgname)
