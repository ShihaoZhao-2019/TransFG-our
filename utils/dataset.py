...
class air():
    def __init__(self, root, is_train=True, data_len=None, transform=None):
        ...
        train_val_file2 = open(os.path.join(self.root, 'images_variant_trainval2.txt'))
        test_val_file2 = open(os.path.join(self.root, 'images_variant_test2.txt'))
        train_img_name_list = []
        train_label_list = []
        for line in train_val_file2:
            train_img_name_list.append(line[:-1].split(' ')[0]+'.jpg')
            train_label_list.append(int(line[:-1].split(' ')[-1]))
        test_img_name_list = []
        test_label_list = []
        for line in test_val_file2:
            test_img_name_list.append(line[:-1].split(' ')[0]+'.jpg')
        ...
        if self.is_train:
            self.train_img = [scipy.misc.imread(os.path.join(self.root, 'images', train_file)) for train_file in
                              train_img_name_list[:data_len]]
            self.train_label = train_label_list
            self.train_imgname = train_img_name_list
            # print(len(self.train_label), len(self.train_imgname))
            # print(self.train_label[34],self.train_imgname[34])
            # print(self.train_label[21], self.train_imgname[21])
            # print(self.train_label[31], self.train_imgname[31])
            # print(self.train_label[88], self.train_imgname[88])
            # print(self.train_label[108], self.train_imgname[108])
            # print(self.train_label[234], self.train_imgname[234])
            # print(self.train_label[352], self.train_imgname[352])
            # print(self.train_label[421], self.train_imgname[421])
        if not self.is_train:
            self.test_img = [scipy.misc.imread(os.path.join(self.root, 'images', test_file)) for test_file in
                             test_img_name_list[:data_len]]
            self.test_label = test_label_list
            self.test_imgname = test_img_name_list
