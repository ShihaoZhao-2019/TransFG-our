...
class SelfData():
    def __init__(self, root, is_train=True, data_len=None, transform=None):
        ...
        for line in img_txt_file:
            img_name_list.append(line[:-1].split(' ')[-1])
            img_id_list.append(line[:-1].split(' ')[0])
        label_list = []
        for line in label_txt_file:
            label_list.append(int(line[:-1].split(' ')[-1]) - 1)
        train_test_list = []
        ...
        train_file_list = [x for i, x in zip(train_test_list, img_name_list) if i]
        test_file_list = [x for i, x in zip(train_test_list, img_name_list) if not i]
        testid_file_list = [x for i, x in zip(train_test_list, img_id_list) if not i]
        trainid_file_list = [x for i, x in zip(train_test_list, img_id_list) if i]
        if self.is_train:
            self.train_img = [imageio.imread(os.path.join(self.root, 'images', train_file)) for train_file in
                              train_file_list[:data_len]]
            self.train_label = [x for i, x in zip(train_test_list, label_list) if i][:data_len]
            self.train_imgname = [x for x in train_file_list[:data_len]]
            self.train_id = [x for x in trainid_file_list[:data_len]]
        if not self.is_train:
            self.test_img = [imageio.imread(os.path.join(self.root, 'images', test_file)) for test_file in
                             test_file_list[:data_len]]
            self.test_label = [x for i, x in zip(train_test_list, label_list) if not i][:data_len]
            self.test_imgname = [x for x in test_file_list[:data_len]]
            self.test_id = [x for x in testid_file_list[:data_len]]
        print('SelfData',len(img_name_list),len(train_file_list),len(test_file_list))
    def __getitem__(self, index):
        if self.is_train:
            img, target, imgname,imgid = self.train_img[index], self.train_label[index], self.train_imgname[index],self.train_id[index]
            if len(img.shape) == 2:
                img = np.stack([img] * 3, 2)
            img = Image.fromarray(img, mode='RGB')
            if self.transform is not None:
                img = self.transform(img)
            gt_labels_one_hot = np.full(100, 0, dtype=np.float16)
            ...
            img, target, imgname,imgid = self.test_img[index], self.test_label[index], self.test_imgname[index],self.test_id[index]
            ...
            return img, target, gt_labels_one_hot, imgid
...
class FGVC_aircraft():
    def __init__(self, input_size, root, is_train=True, data_len=None):
        ...
        print('aircraft',len(self.train_img_label),len(self.test_img_label))
    def __getitem__(self, index):
        if self.is_train:
